...
import { modal, form, buttons, colors, type } from '../styles/shared';
    ...
    return (
        <div
            ...
            style={{
                ...
                backgroundColor: isHovered ? 'rgba(88,166,255,0.05)' : 'transparent',
    ...
    return (
        <div
            style={{
                ...
                borderRadius: '4px',
                overflow: 'hidden',
                border: isHovered ? `2px solid ${colors.danger}` : `1px solid ${colors.border}`,
        ...
        >
            ...
            <button
                ...
                style={{
                    ...
                    top: '-5px',
                    right: '-5px',
                    backgroundColor: isHovered ? colors.danger : 'rgba(248,81,73,0.8)',
                    color: 'white',
                    border: 'none',
                    borderRadius: '50%',
                    width: '18px',
...
const AddProjectModal = ({ isOpen, onClose, onSave, loading, subCounties, editingProject }) => {
    ...
    const validate = () => {
        ...
        if (b < 0 || pc < 0 || ctd < 0) {
            errors.financial = 'Financial values cannot be negative.';
        }
        if (pc > 0 && b > 0 && pc > b) {
            errors.financial = 'Project Cost cannot be greater than Budget.';
        }
        if (ctd > 0 && pc > 0 && ctd > pc) {
            errors.financial = 'Cost to Date cannot be greater than Project Cost.';
        }
        if (cp < 0 || cp > 100) {
            errors.completion_percentage = 'Completion must be between 0 and 100.';
        }
        return errors;
    };
    ...
    return (
        <div style={modal.overlay} onClick={handleClose}>
            <div style={modal.container} onClick={(e) => e.stopPropagation()}>
                <div style={modal.header}>
                    <h2 style={modal.headerTitle}>
                ...
                <form onSubmit={handleSubmit} style={modal.body} noValidate>
                    <div style={modal.formGrid2}>
                        <div style={modal.fieldGroup}>
                            <label style={modal.label}>Project Title <span style={{ color: colors.danger }}>*</span></label>
                            <input
                                name="title"
                            ...
                            />
                            {fieldErrors.title && (
                                <span style={modal.errorText}>{fieldErrors.title}</span>
                            )}
                        </div>
                        <div style={modal.fieldGroup}>
                            <label style={modal.label}>Contractor Name</label>
                            <input
                    ...
                    {fieldErrors.financial && (
                        <div style={{
                            ...type.sm, color: colors.danger, marginBottom: '12px',
                            padding: '8px 12px', backgroundColor: colors.dangerBg,
                            borderRadius: '6px', border: `1px solid ${colors.danger}30`,
                        }}>
                            {fieldErrors.financial}
                        </div>
                    )}
                    ...
                    <div style={modal.formGrid3}>
                        <div style={modal.fieldGroup}>
                            <label style={modal.label}>Budget (Kshs)</label>
                            <input
                        ...
                        <div style={modal.fieldGroup}>
                            <label style={modal.label}>Project Cost (Kshs)</label>
                            <input
                                type="number" name="project_cost" value={formData.project_cost}
                                onChange={handleInputChange} placeholder="0.00"
                                min="0" step="0.01" style={inputStyle}
                            />
                        </div>
                        <div style={modal.fieldGroup}>
                            <label style={modal.label}>Cost to Date (Kshs)</label>
                            <input
                                type="number" name="cost_to_date" value={formData.cost_to_date}
                                onChange={handleInputChange} placeholder="0.00"
                                min="0" step="0.01" style={inputStyle}
                            />
                        </div>
                    ...
                    <div style={modal.formGrid3}>
                        <div style={modal.fieldGroup}>
                            ...
                            <input
                                type="number" name="completion_percentage"
                                value={formData.completion_percentage}
                                onChange={handleInputChange}
                                min="0" max="100" style={inputStyle}
                            />
                            {fieldErrors.completion_percentage && (
                                <span style={modal.errorText}>{fieldErrors.completion_percentage}</span>
                            )}
                        </div>
                        <div style={modal.fieldGroup}>
                    ...
                    <div style={modal.formGrid3}>
                        <div style={modal.fieldGroup}>
                            ...
                            <input
                                ...
                                onChange={handleInputChange} placeholder="e.g. Road, Bridge"
                                style={inputStyle}
                            />
                        </div>
                        <div style={modal.fieldGroup}>
                        ...
                        <div style={modal.fieldGroup}>
                            ...
                            {availableWards.length > 0 ? (
                                ...
                                <input
                                    name="ward_area" value={formData.ward_area}
                                    onChange={handleInputChange}
                                    placeholder="Select sub-county first"
                                    disabled={!formData.sub_county}
                                    style={{ ...inputStyle, opacity: formData.sub_county ? 1 : 0.5 }}
                                />
                            )}
                        </div>
                    </div>
                    ...
                    <div style={modal.actions}>
                        <button type="button" onClick={handleClose} style={buttons.secondary}>
                            Cancel
                        </button>
